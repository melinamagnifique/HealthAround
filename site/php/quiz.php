      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          if (response.isCorrect === true || response.isCorrect === "true") {
            ...
            $answerFeedback.html("La bonne réponse est : " + response.correctAnswer).css("color", "green");
          }

          var funFact = "";
          if (question === "température") {
            funFact = "Fun Fact : Saviez-vous que des températures plus élevées à l'intérieur du train peuvent augmenter la fatigue du conducteur et affecter sa concentration.";
          } else if (question === "niveau_CO2") {
            funFact = "Fun Fact : Saviez-vous que les niveaux élevés de CO2 à l'intérieur des trains peuvent être attribués à la respiration des passagers et à la circulation de l'air limitée.";
          }else if (question === "niveau_bruit"){
            funFact = "Fun Fact : Les niveaux de bruit à l'intérieur du métro peuvent atteindre des niveaux élevés, en particulier lors des arrêts et des départs. Saviez-vous que ces niveaux sonores peuvent être comparables à ceux d'un avion au décollage ? Les ingénieurs travaillent continuellement pour réduire le bruit à l'intérieur des véhicules et des stations de métro.";
          }

          $funFact.html(funFact);
        },
        error: function() {
          $responseMessage.html("Une erreur s'est produite lors du traitement de la requête.").css("color", "red");
        }
      });
    });
  });
