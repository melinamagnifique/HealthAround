...
<ul class="u-nav u-unstyled u-nav-1"><li class="u-nav-item"><a class="u-button-style u-nav-link u-text-active-palette-1-base u-text-hover-custom-color-1" href="../php/accueil_admin.php" style="padding: 10px 20px;">Accueil Admin</a>
  ...
  <center>
        <div class="admin-section">
            <h2>Bienvenue sur la page admin des seuils de capteurs</h2>
            <h3>Retrouvez toutes les infos des mesures enregistrées par nos capteurs</h3>

</center>
<center><h4>Voici les seuils des différents capteurs, modifiez-les si besoin</h4></center>

  <?php
  // Tableau de types de capteurs
  $capteurs = array(
    array('type' => 'Capteur cardiaque', 'seuil' => 100, 'unité' => 'bpm'),
    array('type' => 'Capteur d\'humidité et température', 'seuil' => 50, 'unité' => '°C'),
    array('type' => 'Capteur de son', 'seuil' => 80, 'unité' => 'dB'),
    array('type' => 'Capteur de CO2', 'seuil' => 500, 'unité' => 'ppm')
);

  // Vérifier si le formulaire a été soumis
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Parcourir les valeurs soumises et mettre à jour les seuils correspondants
    foreach ($_POST as $key => $value) {
      // Vérifier si la clé correspond à un seuil
      if (strpos($key, 'seuil_') === 0) {
        $index = substr($key, strlen('seuil_'));
        // Vérifier si l'index est valide
        if (isset($capteurs[$index])) {
          $capteurs[$index]['seuil'] = $value;
        }
      }
    }
  }
  ...
  <form method="POST">
    <table>
      <tr>
        ...
        <th>Seuil</th>
        <th>Modifier le seuil</th>
        <th>Enregistrer</th>
      </tr>
      <?php foreach ($capteurs as $index => $capteur) { ?>
        <tr>
          <td><?php echo $capteur['type']; ?></td>
          <td><?php echo $capteur['seuil']. ' ' . $capteur['unité']; ?></td>
          <input type="hidden" name="idSeuil_<?php echo $index; ?>" value="<?php echo $index; ?>">
          <td><input type="range" min="0" max="500" value="<?php echo $capteur['seuil']; ?>" name="seuil_<?php echo $index; ?>"></td>
          <td><input type="submit" value="Enregistrer" onclick="confirmation()"></td>
        </tr>
      <?php } ?>
    </table>
  </form>
  ...
  if (confirm("Êtes-vous sûr de vouloir modifier les seuils ?")) {
    // Le formulaire sera soumis si l'utilisateur clique sur "OK"
    return true;
  } else {
    // Le formulaire ne sera pas soumis si l'utilisateur clique sur "Annuler"
    return false;
  }
}
</script>
